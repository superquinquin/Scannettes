...
class Purchase:
    ...
    def __init__(
        ...
        if supplier is None:
            ...
            self.supplier = Supplier(supplier.id, supplier.name)
    ...
    def active_tables(self) -> List[str]:
        ...
        return [
            tb
            for tb in ["table_entries", "table_queue", "table_done"]
    ...
    def get_item_origin_index(self, product: dict) -> Union[None, int]:
        ...
        origin_idx = None
        p = self.table.loc[
            (self.table["barcode"] == product["barcode"])
            | (self.table["id"] == product["id"])
        ]
        if not p.empty:
            origin_idx = p.index.tolist()[0]

        return origin_idx

    def get_item_location_index(
        self, product: dict
    ) -> Tuple[Union[None, int], Union[None, str]]:
    ...
    def update_item(self, table_name: str, idx: int, product: dict) -> None:
        ...
        table.loc[idx, ["barcode", "name", "qty", "pckg_qty"]] = [
            product["barcode"],
            product["name"],
            product["qty"],
            product["qty_packaqe"],
    ...
    def recharge_item(
        self, product: dict, state: Union[None, str], item_state: str
    ) -> None:
    ...
    ####################################################################
    ########################### SEARCH SCANNED ITEM ####################
    def _get_product_out_of_exceptions(
        self, odoo: object, model: str, context: dict
    ) -> dict:
        products = odoo.browse(model, [("barcode", "=", context["code_ean"])])
        product = products[0]
        for pr in products:
            if (model == "product.product" and context["code_ean"] == pr.barcode) or (
                model == "product.multi.barcode"
                and context["code_ean"] == pr.barcode
                and context["code_ean"] == pr.product_id.barcode
            ):
                product = pr
                break
    ...
    def _state_1_search(self, odoo: object, context: dict) -> dict:
        ...
        try:
            alt = odoo.get(
                "product.multi.barcode", [("barcode", "=", context["code_ean"])]
            )

        except ValueError:
            # just select first one in this case
            alt = self._get_product_out_of_exceptions(
                odoo, "product.multi.barcode", context
            )
        ...
        if not alt:
            ...
            self.table_entries[
                (self.table_entries["barcode"] == alt.product_id.barcode)
            ].empty
            == False
        ):
            # found product
            ...
            self.table_entries[(self.table_entries["barcode"] == alt.barcode)].empty
            == False
        ):
            ## find product with alt barcode
            context["product"] = self.table_entries[
                (self.table_entries["barcode"] == alt.barcode)
            ]
    ...
    def _state_2_search(self, odoo: object, context: dict) -> dict:
        ...
        try:
            ...
            context["product"] = odoo.get(
                "product.product", [("barcode", "=", context["code_ean"])]
            )

        except ValueError:
            # WHEN MULTIPLE APPEARANCE IS FOUND
            # TRY TO SELECT PRODUCT WITH MAIN BARCODE
            context["product"] = self._get_product_out_of_exceptions(
                odoo, "product.product", context
            )
    ...
    def _move_scanned_item(self, odoo: object, context: dict) -> dict:
        ...
        if context["state"] in [0, 1]:
            ...
            new_item = False
            self.table_queue = pd.concat(
                [self.table_queue, context["product"]], ignore_index=True
            )
            self.table_entries = self.table_entries.drop(
                context["product"].index, axis=0
            )
            ...
            placeholder = pd.DataFrame(
                [[context["code_ean"], id, name, qty, pkg_qty, received_qty]],
                columns=columns,
            )
            placeholder.fillna(0, inplace=True)
            self.table_queue = pd.concat(
                [self.table_queue, placeholder], ignore_index=True
            )
    ...
    def assembler(self, other: object):
        ...
        def merge_list():
            ...
            self.scanned_barcodes = list(
                set(self.scanned_barcodes + other.scanned_barcodes)
            )
        ...
        def merge_qty():
            v = self.table_done.loc[
                self.table_done["id"] == record["id"], "qty_received"
            ]
            self.table_done.loc[
                self.table_done["id"] == record["id"], "qty_received"
            ] = int(v) + int(record["qty_received"])

        def drop_duplicate_row():
            if self.table_entries.loc[
                self.table_entries["id"] == record["id"], "qty_received"
            ].any():
                self.table_entries = self.table_entries.drop(
                    self.table_entries.loc[
                        self.table_entries["id"] == record["id"], "qty_received"
                    ].index
                )

            if self.table_queue.loc[
                self.table_queue["id"] == record["id"], "qty_received"
            ].any():
                self.table_queue = self.table_queue.drop(
                    self.table_queue.loc[
                        self.table_queue["id"] == record["id"], "qty_received"
                    ].index
                )
    ...
    def nullifier(self):
        ...
        self.table_done = pd.concat(
            [self.table_done, self.table_queue, self.table_entries]
        )
        self.table_done = self.table_done.reset_index(drop=True)
