...
class PDF(FPDF):
  ...
  def draw_qrcode_box(self, x:float, y:float, qrcode, caption):
    ...
    self.cell(0, 7, f"**numéro de commande :** {str(caption['id'])}", 
              markdown=True)
    self.set_xy(x+65,y+15)
    self.multi_cell(0, 7, self.break_string(90, f"**fournisseur :** {caption['supplier']}"), 
                    markdown=True) #
    ...
    if len(str) > line_break:
      while spliter:
        ...
        for i in range(len(copy)):
          ...
          if (i != 0 and 
              i % line_break == 0 and space_idx != 0):
  ...
  def generate_pdf(self, qrcode:list, caption:list):
    ...
    for i in range(len(qrcode)):
      ...
      if place == 0:
        ...
        self.draw_qrcode_box(20, y_sep + place*50 + place*y_sep, qrcode[i], caption[i])
        space -= y_sep + 50
        place += 1
