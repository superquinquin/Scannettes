...
from cannettes.packages.purchase import Purchase, Supplier
from cannettes.packages.lobby import Lobby
from cannettes.packages.utils import get_ceiling_date, format_error_cases
...
class Odoo:
    ...
    def get_barcode(self, item) -> int:
        ...
        alt = self.browse(
            "product.multi.barcode", [("product_id", "=", item.product_id.id)]
        )
    ...
    def create_inventory(self, input: dict, data: dict, table: pd.DataFrame) -> int:
        ...
        inventory = Purchase(
            object_id,
            [x[0] for x in data["odoo"]["inventory"]["type"] if x[1] == cat_id][
                0
